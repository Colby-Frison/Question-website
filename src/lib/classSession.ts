...
/**
 * Listen for the count of students in a class session
 * 
 * Sets up a real-time listener for the number of students who have joined a specific session.
 * This is useful for professors to see how many students are currently in their class.
 * 
 * @param sessionCode - The session code to track student count for
 * @param callback - Function to call when the student count changes
...
export const listenForStudentCount = (
  ...
  try {
    console.log(`Setting up student count listener for session: ${sessionCode}`);
    
    // Query for students who have joined this session
    const q = query(
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        console.log(`Session ${sessionCode} has ${count} active students`);
        callback(count);
      },
      (error) => {
    ...
    return () => {
      console.log(`Cleaning up student count listener for session: ${sessionCode}`);
      unsubscribe();
    };
  } catch (error) {
