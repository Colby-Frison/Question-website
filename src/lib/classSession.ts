...
/**
 * Get the most recent active session for a professor
 * 
 * @param professorId - The ID of the professor
 * @returns A promise that resolves to the most recent active session or null if none found
 */
export const getMostRecentActiveSession = async (professorId: string): Promise<ClassSession | null> => {
  if (!professorId) {
    console.warn("No professor ID provided to getMostRecentActiveSession");
    return null;
  }

  try {
    console.log(`Fetching most recent active session for professor: ${professorId}`);
    const q = query(
      collection(db, CLASS_SESSIONS_COLLECTION),
    ...
    if (querySnapshot.empty) {
      console.log(`No active sessions found for professor ${professorId}`);
      return null;
    }
    ...
    return {
      id: doc.id,
      code: data.code,
      sessionCode: data.sessionCode || doc.id.substring(0, 6),
