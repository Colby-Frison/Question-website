...
const ClassQuestionList: React.FC<ClassQuestionListProps> = React.memo(({
    ...
    if (isLoading) {
      return (
        <div className="flex items-center justify-center py-6 sm:py-8">
          ...
          <span className="ml-2 text-xs sm:text-sm text-text-secondary">Loading class questions...</span>
        </div>
      );
    }
    return null;
  ...
  const renderEmptyState = useMemo(() => {
    if (questions.length === 0) {
      return (
        <div className="rounded-md bg-background-secondary p-4 sm:p-8 text-center">
          <p className="text-sm sm:text-base text-text-secondary">No questions have been asked in class yet.</p>
        </div>
      );
    }
    return null;
  ...
  const questionItems = useMemo(() => {
    return questions.map((question) => (
      <li key={question.id} className="py-3 sm:py-4 border-b border-background-tertiary last:border-0 relative">
        {/* Status indicator */}
        ...
        <div className="flex flex-col space-y-2 w-full">
          <div className="w-full">
            <div className="text-sm sm:text-base text-text break-words whitespace-normal overflow-wrap-anywhere pr-8 pl-2">
              {question.text}
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col h-full">
      <h2 className="text-lg font-semibold mb-4 text-text">Class Questions</h2>
      <ul className="divide-y divide-background-tertiary rounded-md bg-white p-2 sm:p-4 w-full overflow-hidden">
        {questionItems}
      </ul>
    </div>
  );
